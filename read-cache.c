...
void rename_index_entry_at(struct index_state *istate, int nr, const char *new_name)
{
	...
	new_entry = mem_pool__ce_alloc(find_mem_pool(istate), namelen);
					   ...
					   struct cache_entry *alias)
{
	...
	new_entry = make_empty_cache_entry_from_index(istate, len);
...
int add_to_index(struct index_state *istate, const char *path, struct stat *st, int flags)
{
	...
	ce = make_empty_cache_entry_from_index(istate, namelen);
...
struct cache_entry *make_empty_cache_entry_from_index(struct index_state *istate, size_t len)
{
					     ...
					     int *changed_ret)
{
	...
	updated = make_empty_cache_entry_from_index(istate, ce_namelen(ce));
...
int read_index_unmerged(struct index_state *istate)
{
	...
	for (i = 0; i < istate->cache_nr; i++) {
		...
		unmerged = 1;
		len = ce_namelen(ce);
		new_ce = make_empty_cache_entry_from_index(istate, len);
