...
static int wt_deserialize_v1_header(struct wt_status *s, int fd)
{
	...
	while ((line = my_packet_read_line(fd, &line_len))) {
		...
		if (skip_prefix(line, "detect_break ", &arg)) {
			s->detect_break = (int)strtol(arg, NULL, 10);
			continue;
		}
		/* status_format */
		...
		if (skip_prefix(line, "commitable ", &arg)) {
			s->commitable = (int)strtol(arg, NULL, 10);
			continue;
		}
...
static int wt_deserialize_fd(const struct wt_status *cmd_s, struct wt_status *des_s, int fd)
{
	...
	if (cmd_s->detect_break != des_s->detect_break) {
		trace_printf_key(&trace_deserialize, "reject: detect_break");
		return DESERIALIZE_ERR;
	}
	/* status_format */
