...
static void find_short_packed_object(struct disambiguate_state *ds)
{
	...
	prepare_packed_git_internal(the_repository, 1);
				     ...
				     struct min_abbrev_data *mad)
{
	...
	match = bsearch_midx(m, mad->oid->hash, &first);

	/*
	 * first is now the position in the packfile where we would insert
	 * mad->oid->hash if it does not exist (or the position of
	 * mad->oid->hash if it does exist). Hence, we consider a maximum of
	 * three objects nearby for the abbreviation length.
	 */
	mad->init_len = 0;
				     ...
				     struct min_abbrev_data *mad)
{
	 ...
	 * first is now the position in the packfile where we would insert
	 * mad->oid->hash if it does not exist (or the position of
	 * mad->oid->hash if it does exist). Hence, we consider a maximum of two
	 * objects nearby for the abbreviation length.
	 */
	mad->init_len = 0;
	if (!match) {
