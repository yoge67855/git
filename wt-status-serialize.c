...
static void wt_serialize_v1_header(struct wt_status *s, int fd)
{
	...
	packet_write_fmt(fd, "detect_break %d\n", s->detect_break);
	/* status_format */
 ...
 * We write raw (not c-quoted) pathname(s).  The head_path is only
					   ...
					   struct string_list_item *item)
{
	...
	/*
	 * Write <path> NUL [<head_path>] NUL LF at the end of the buffer.
	 */
	len_path = strlen(item->string);
	len_head_path = d->head_path ? strlen(d->head_path) : 0;

	/*
	 * This is a bit of a hack, but I don't want to split the
	 * status detail record across multiple pkt-lines.
	 */
	if (p + len_path + 1 + len_head_path + 1 + 1 >= end)
	...
	*p++ = '\0';
	...
	if (len_head_path) {
		memcpy(p, d->head_path, len_head_path);
		p += len_head_path;
	}
	*p++ = '\0';
	*p++ = '\n';
